...
<div class="panel-main">
    <div class="panel-header">
        <div class="panel-header-actions">
            <h1>Authors</h1>
            <a href="/bookmart/authors/add_author.php"> <img src="/bookmart/public/images/add.svg" /></a>
            <a href=<?=isset($_POST['submit']) ? "/bookmart/exportcsv.php?table=tbl_Author&filter=true&key=" . urlencode($columnMap[$_POST['key']]) . "&operator=" . urlencode($_POST['operator']) . "&value=" . urlencode($_POST['value']) : "/bookmart/exportcsv.php?table=tbl_Author&filter=false"?>><img src="/bookmart/public/images/exportcsv.svg" /></a>
        </div>
            ...
            <div class="row">
                ...
                <div class="cell" data-title="Status">
                    <div class="dropdown-status">
                        <span id="items-link" style='color:<?=$row['A_status'] == "active" ? "#002460" : "red"?>'><?=$row['A_status'] == "active" ? "active" : "inactive"?><img id="dropdownArrow" src="/bookmart/public/images/<?=$row['A_status'] == "active" ? "dropdownArrowBlue.svg" : "dropdownArrowRed.svg"?>" /></span>
                        <div class="dropdown-status-content">
                            <a href="/bookmart/authors/change_status.php?id=<?=$row['Author_id']?>" style='color:<?=$row['A_status'] == "active" ? "red" : "#002460"?>'><?php echo $row['A_status'] == "active" ? "inactive" : "active" ?></a>
                        </div>
                    </div>
                </div>
                <div class="cell" data-title="Actions">
                    <div class="table-actions">
                        <a href="/bookmart/authors/edit_author.php?id=<?=$row['Author_id']?>"><img src="/bookmart/public/images/edit.svg" /></a>
                    </div>
                </div>
            </div>
            <?php endwhile?>
...
function observerCallback(entries, observer) {
    entries.forEach((entry) => {
        if (entry.isIntersecting) {
            ...
            fetch(<?=isset($_POST['submit']) ? "`/bookmart/authors?filter=true&key={$_POST['key']}&value={$_POST['value']}&operator={$_POST['operator']}&offset=" . '${offset}`' : "`/bookmart/authors?filter=false&offset=" . '${offset}`'?>)
...
class Row {
    constructor(data) {
        ...
        const rowHtml = `
            ...
            <div class="cell" data-title="Status">
                <div class="dropdown-status">
                    <span id="items-link" style='color:${data['A_status']=="active"?"#002460":"red"}'>${data['A_status']}<img id="dropdownArrow" src="/bookmart/public/images/${data['A_status']=="active"?"dropdownArrowBlue.svg":"dropdownArrowRed.svg"}" /></span>
                    <div class="dropdown-status-content">
                    <a href="/bookmart/authors/change_status.php?id=${data['Author_id']}" style='color:${data['A_status']=="active"?"red":"#002460"}'>${data['A_status']=="active"?"inactive":"active"}</a>
                    </div>
                </div>
            </div>
            <div class="cell" data-title="Actions">
                <div class="table-actions">
                    <a href="/bookmart/authors/edit_author.php?id=${data['Author_id']}"><img src="/bookmart/public/images/edit.svg" /></a>
                </div>
            </div>
        `
