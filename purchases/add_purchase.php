...
class PurchaseTable {
    ...
    <div class="input-textfield">
        <input type="number" class="form-textfield" name="Purchase_price" required id="purchase-price" />
    ...
    <div class="input-textfield">
        <input type="number" class="form-textfield" name="profitpercentage" id="profit-percentage" required />
        <span class="floating-label">Profit Percentage</span>
        <p id="profitpercentage-error-div"></p>
    </div>
    <div class="input-textfield">
        <input type="number" class="form-textfield" name="sellingprice" id="selling-price" readonly />
        <span class="floating-label-selling">Selling Price</span>
    </div>
    <div class="input-textfield">
        ...
        div.querySelectorAll('.form-textfield').forEach(field => {
            field.addEventListener('input', e => {
                ...
                var errorP, regex, errorMessage;
                if (fieldName == 'Purchase_price') {
                    errorP = div.querySelector('#purchase-price-error-div');
                    regex = /^[0-9.]{1,11}$/;
                    errorMessage = 'Invalid purchase price';
                } else if (fieldName == 'quantity') {
                    errorP = div.querySelector('#quantity-error-div');
                    regex = /^[0-9]{1,5}$/;
                    errorMessage = 'Invalid quantity';
                } else {
                    errorP = div.querySelector('#profitpercentage-error-div');
                    regex = /^-?[0-9]{1,9}$/;
                    errorMessage = 'Invalid percentage';
                }

                const purchaseprice = div.querySelector('#purchase-price').value;
                const profitpercentage = div.querySelector('#profit-percentage').value;

                if (fieldName == "profitpercentage" && purchaseprice) {
                    if (!input) {
                        div.querySelector('#selling-price').value = '';
                    } else {
                        div.querySelector('#selling-price').value = parseInt(purchaseprice) + ((profitpercentage / 100) * purchaseprice);
                    }
                } else if (fieldName == "Purchase_price" && profitpercentage) {
                    if (!input) {
                        div.querySelector('#selling-price').value = '';
                    } else {
                        div.querySelector('#selling-price').value = parseInt(purchaseprice) + ((profitpercentage / 100) * purchaseprice);
