...
<div class="panel-main">
    <div class="panel-header">
        <div class="panel-header-actions">
            <h1>Purchases</h1>
            <a href="/bookmart/purchases/add_purchase.php"> <img src="/bookmart/public/images/add.svg" /></a>
            <a href=<?=isset($_POST['submit']) ? "/bookmart/exportcsv.php?table=tbl_Purchase_master&filter=true&key=" . urlencode($columnMap[$_POST['key']]) . "&operator=" . urlencode($_POST['operator']) . "&value=" . urlencode($_POST['value']) : "/bookmart/exportcsv.php?table=tbl_Purchase_master&filter=false"?>><img src="/bookmart/public/images/exportcsv.svg" /></a>
        </div>
...
function observerCallback(entries, observer) {
    entries.forEach((entry) => {
        if (entry.isIntersecting) {
            ...
            fetch(<?=isset($_POST['submit']) ? "`/bookmart/purchases?filter=true&key={$_POST['key']}&value={$_POST['value']}&operator={$_POST['operator']}&offset=" . '${offset}`' : "`/bookmart/purchases?filter=false&offset=" . '${offset}`'?>)
